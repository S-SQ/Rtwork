...
points <- data.frame(x = numeric(n), y = numeric(n), dir = numeric(n), level = integer(n)) # Dataframe of 0s of n length
edges <-  data.frame(x = numeric(n), y = numeric(n), xend = numeric(n), yend = numeric(n), level = integer(n)) # Dataframe of 0s of n length
...
if(initial_pts > 1) {
  i <- 1 
  while(i <= initial_pts) {
    points[i, ] <- c(runif(1, 0, width), # selects uniform between 0 and width
                     runif(1, 0, height),  # selects uniform between 0 and height 
                     runif(1, -2*pi, 2*pi), # selects uniform between -2*pi and 2*pi
                     1) # Assigns value of 1
    i <- i + 1 # increase i
  }
}
t0 <- Sys.time()
...
# print plot
ggsave("plot001.png", p2, width = 20, height = 20, units = "cm", dpi = 300)
...
points(sin(a)*r, cos(a)*r, cex=3*r, pch=16,
